...
class SolutionDay21(bs.BaseSolution):
    def __init__(self, day_num: int, example=False, verbose=False):
        ...
        # But we actually need the next-level path length...
        self.dir_pad2_path_len = dict()
        # Compute dir_pad_paths:
        for start in self.dir_pad_paths.keys():
            self.dir_pad2_path_len[start] = dict()
            for end in self.dir_pad_paths[start].keys():
                path_length = []
                for path in self.dir_pad_paths[start][end]:
                    path = 'A' + path + 'A'
                    path_length.append(0)
                    for i in range(1, len(path)):
                        x, y = path[i - 1], path[i]
                        # Compute the dir_pad_path_len from the previous position x to the current position y. Plus pressing the button
                        path_length[-1] += self.dir_pad_path_len[x][y] + 1

                self.dir_pad2_path_len[start][end] = min(path_length)
        ...
        # Compute the second half of it (i.e. other diagonal)
        ...
        self.num_pad_path_len = dict()
        # Compute dir_pad_paths:
        for start in self.num_pad_paths.keys():
            self.num_pad_path_len[start] = dict()
            for end in self.num_pad_paths[start].keys():
                path_length = []
                for path in self.num_pad_paths[start][end]:
                    path = 'A' + path + 'A'
                    path_length.append(0)
                    for i in range(1, len(path)):
                        x, y = path[i - 1], path[i]
                        # Compute the dir_pad_path_len from the previous position x to the current position y.
                        path_length[-1] += self.dir_pad2_path_len[x][y]

                self.num_pad_path_len[start][end] = min(path_length)

    def _test(self):
        """Some tests used to develop the super-complicated pre-processing in the _init_() funciton...
        ...
        total_length = self.compute_shortest_path('029A')
    ...
    def compute_shortest_path(self, num_input):
        path = 'A' + num_input
        total_length = 0
        for i in range(1, len(path)):
            x, y = path[i - 1], path[i]
    ...
    def _star_1(self) -> int:
        ...
        with open(input_file_path, 'r') as of:
            while line := of.readline().strip():
                length = self.compute_shortest_path(line)
